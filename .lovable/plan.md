...
7. **Hero page accent colour** — change the marketing/hero nav menu links (`src/components/marketing-nav.tsx`) and the "No more blind spots" headline (`src/components/auth-shell.tsx`) from `#DF2F2F` to a light orange (Medium Tangerine, around `#F2A93B`) so they pop against the navy background.
