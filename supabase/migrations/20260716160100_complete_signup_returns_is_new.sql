-- complete_signup previously returned only the retailer id, so callers
-- couldn't tell a brand-new signup (just provisioned a retailer) apart from
-- an existing user it silently no-op'd for. handleSignIn calls this on
-- every sign-in (it's the deferred-provisioning path for accounts that
-- needed email confirmation), so without this the client had no way to
-- route only first-time confirmations into the TAG Setup wizard.
--
-- `provisioned_new_retailer` is true only when this call created a brand
-- new retailer (the account owner's first-ever signup) — not when it just
-- attached a staff invite to an existing, already-configured retailer,
-- since a joining staff member shouldn't be sent through the "connect your
-- POS system and import your catalogue" wizard meant for the owner.
