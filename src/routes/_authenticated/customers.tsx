            ...
            {list.isLoading ? (
              ...
              <div className="p-6">
                <EmptyState
                  ...
                  description="Customers appear here after their first QR scan and opt-in."
                />
              </div>
            ) : (
              (list.data!.rows as any[]).map((c) => (
                ...
                >
                  ...
                  <span className="text-center text-sm tabular-nums">
                    {money(c.lifetime_revenue_cents)}
                  </span>
          ...
          {(list.data?.total ?? 0) > 25 && (
            <div className="flex items-center justify-between border-t px-6 py-3 text-sm">
              <span className="text-muted-foreground">
                Page {page} of {Math.ceil((list.data?.total ?? 0) / 25)}
              </span>
        ...
        {!c ? (
          ...
          <>
            <SheetHeader>
              ...
              <SheetDescription>
                {c.whatsapp_e164} · since {new Date(c.created_at).toLocaleDateString()}
              </SheetDescription>
            </SheetHeader>
            ...
            <Section title="Interests">
              {detail.data!.interests.length === 0 ? (
                ...
                detail.data!.interests.map((i: any) => (
                  <div key={i.id} className="flex items-center gap-3 rounded-lg border p-2">
                    {i.product?.image_url ? (
                      <img src={i.product.image_url} className="h-8 w-8 rounded object-cover" />
                    ) : (
            ...
            <Section title="Recent scans">
              {detail.data!.scans.length === 0 ? (
                <p className="text-sm text-muted-foreground">No scans yet.</p>
              ) : (
                detail.data!.scans.slice(0, 8).map((s: any) => (
            ...
            <Section title="Conversations">
              {detail.data!.conversations.length === 0 ? (
                ...
                detail.data!.conversations.map((cv: any) => (
                  <Link
                    to="/inbox"
                    key={cv.id}
                    className="flex items-center justify-between rounded-lg border p-2 hover:bg-muted/40"
                  >
