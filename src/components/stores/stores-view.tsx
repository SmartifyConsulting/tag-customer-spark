...
const ALL = "__all__";
const VIEW_STORAGE_KEY = "tag.stores.viewMode";
type ViewMode = "grid" | "list";

// Reads the last-picked grid/list preference so the layout persists
// across sessions. SSR-safe: falls back to grid until hydration.
function useStoresViewMode(): [ViewMode, (m: ViewMode) => void] {
...
export function StoresView() {
  ...
  const [viewMode, setViewMode] = useStoresViewMode();
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {hasFilters && (
            <span className="text-sm text-muted-foreground">
              {filtered.length} of {allStores.length}
            </span>
          )}
        </div>
      )}
      ...
      {stores.isLoading ? (
        ...
        <EmptyState
          icon={Store}
          title="No stores yet"
          description="Add your first physical store to start assigning staff and tracking engagement."
        />
      ) : filtered.length === 0 ? (
            ...
            <Card key={s.id} className="rounded-2xl">
              <CardContent className="space-y-4 p-5">
                  ...
                  <div className="flex items-center gap-1.5 text-foreground">
                    ...
                    {s.contact_phone ? (
                      <a
                        ...
                        className="truncate text-[color:var(--mint)] hover:underline"
                      >
                        {s.contact_phone}
                      </a>
                    ) : (
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="rounded-lg border p-2">
                    <p className="text-xs text-muted-foreground">
                      <Users className="mx-auto h-3 w-3" />
                    </p>
            ...
            <tbody className="divide-y">
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-2 text-muted-foreground">{s.manager_name || "—"}</td>
                  <td className="px-4 py-2 text-muted-foreground">
                    {s.contact_phone ? (
                      <a href={`tel:${s.contact_phone}`} className="text-[color:var(--mint)] hover:underline">
                        {s.contact_phone}
                      </a>
                    ) : (
                      "—"
                  ...
                  <td className="px-4 py-2 text-right">{s.scans}</td>
                  <td className="px-4 py-2 text-right">{s.staff_count}</td>
                  <td className="px-4 py-2 text-right">{money(s.recovered_cents)}</td>
