      ...
      {NAV.map((item) => {
        ...
        return (
          <Link
            key={item.url}
            {...(linkProps as any)}
            className="flex items-center gap-2 rounded-md bg-[color:var(--mint)] px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-gray-200 hover:text-[#A6446B]"
          >
            {item.title}
...
function AdminGroup({ item, pathname }: { item: NavItem; pathname: string }) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="flex items-center gap-2 rounded-md bg-[color:var(--mint)] px-4 py-2 text-sm font-bold text-white outline-none transition-colors hover:bg-gray-200 hover:text-[#A6446B]">
        {item.title}
