...
export type CampaignInitial = Partial<{
  id: string;
  title: string;
...
export function CampaignComposer({
  ...
  const [type, setType] = useState<CampaignInput["type"]>(initial?.type ?? "sale");
  const [title, setTitle] = useState(initial?.title ?? "New campaign");
  const [productId, setProductId] = useState<string | undefined>(initial?.product_id ?? undefined);
  const [headline, setHeadline] = useState(initial?.headline ?? "");
  const [body, setBody] = useState(initial?.body ?? "");
  const [ctaLabel, setCtaLabel] = useState(initial?.cta_label ?? "View deal");
  const [ctaUrl, setCtaUrl] = useState(initial?.cta_url ?? "");
  const [imageUrl, setImageUrl] = useState<string | undefined>(initial?.image_url ?? undefined);
  ...
  async function persist(action: "draft" | "send" | "schedule") {
    ...
    try {
      ...
      if (action === "send") await enqueue({ data: { id, sendNow: true } });
      else if (action === "schedule") await enqueue({ data: { id, sendNow: false } });
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  {product ? <span className="font-medium text-foreground">{product.name}</span> : "all products"}.
                </p>
              </div>
          ...
          <Card className="p-5 space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3 items-end">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => persist("draft")} disabled={saving !== "none"}>
                  {saving === "draft" ? (
                ...
                <Button variant="outline" onClick={() => persist("schedule")} disabled={saving !== "none" || !scheduledAt}>
                  {saving === "schedule" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <>
                ...
                <Button onClick={() => persist("send")} disabled={saving !== "none" || audience === 0}>
                  {saving === "send" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <>
