...
import { getProductIntent, recomputeProductIntentFn } from "@/lib/intent.functions";
...
export function ProductIntentPanel({ productId }: { productId: string }) {
  ...
  // Build forecast trajectory for the chart
  const chartData = [...history];
  if (fc?.predicted_score_7d != null) {
    const lastDate = history.at(-1)?.date ?? new Date().toISOString().slice(0,10);
    const d7 = new Date(lastDate); d7.setDate(d7.getDate() + 7);
    const d14 = new Date(lastDate); d14.setDate(d14.getDate() + 14);
    chartData.push({ date: d7.toISOString().slice(0,10), score: Number(fc.predicted_score_7d), forecast: true });
    chartData.push({ date: d14.toISOString().slice(0,10), score: Number(fc.predicted_score_14d), forecast: true });
  }
  ...
  const TrendIcon = p.intent_score_trend === "rising" ? TrendingUp
    : p.intent_score_trend === "falling" ? TrendingDown : Minus;

  const insight = generateInsight(p, sig, fc);
    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
        <div>
          ...
          <CardDescription>Aggregated demand signal · updates in real time.</CardDescription>
        </div>
        ...
        <div className="flex items-center gap-6 flex-wrap">
          <div>
            <div className="text-5xl font-bold tabular-nums leading-none">
              {Math.round(Number(p.intent_score))}
            </div>
            <div className="text-xs text-muted-foreground mt-1">out of 100</div>
          </div>
          <div className="space-y-2">
            <IntentBadge score={p.intent_score} trend={p.intent_score_trend} confidence={p.intent_score_confidence} size="lg" />
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <TrendIcon className="h-3 w-3" />
              <span className="capitalize">{p.intent_score_trend}</span>
              <span>·</span>
              <span>confidence {(Number(p.intent_score_confidence) * 100).toFixed(0)}%</span>
            </div>
          </div>
          {fc?.predicted_trend && (
            <div className="ml-auto rounded-lg border p-3 text-sm">
              ...
              <div className="font-semibold">7-day: {Math.round(Number(fc.predicted_score_7d))} · 14-day: {Math.round(Number(fc.predicted_score_14d))}</div>
              <div className="text-xs text-muted-foreground capitalize">Trend: {fc.predicted_trend}</div>
            </div>
          )}
        </div>
...
function generateInsight(p: any, sig: any, fc: any): string {
