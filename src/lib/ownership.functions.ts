...
import { resolveRetailerId, resolveOwnershipContext } from "@/lib/ownership-context.server";
import { assertReceiptsEnabled } from "@/lib/system-settings.functions";
...
export const listPurchases = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context as any;
    await assertReceiptsEnabled(supabase, userId);
    const { retailerId, tagRef } = await resolveOwnershipContext(supabase, userId);
